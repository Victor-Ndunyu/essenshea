...
test('every storefront surface loads the final calm-luxury design layer', async () => {
  for (const page of pages) {
    ...
    assert.match(html, /luxury-refinement\.css\?v=20/, `${page} is missing the current design layer`);
...
test('fashion atelier system reaches forms, overlays, scrollbar and reduced motion', async () => {
  ...
  assert.match(css, /body:has\(\.site-footer\)[\s\S]*overflow-y:\s*visible/);
  assert.match(css, /\.topbar\s*\{[\s\S]*position:\s*fixed/);
  assert.match(css, /\.topbar\.is-hidden[\s\S]*translateY\(calc\(-100% - 14px\)\)/);
  assert.match(css, /\.hero__copy\s*\{[\s\S]*margin:\s*28px 0 34px clamp\(36px, 4vw, 68px\)/);
...
test('shop hero presents a layered product lookbook with restrained pointer motion', async () => {
  ...
  assert.match(script, /hideShopPageLoader/);
  assert.match(css, /\.shop-page-loader\s*\{/);
});
