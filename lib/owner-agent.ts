...
async function summarizeSite(): Promise<string> {
  ...
  const lines = ['Yes. Current public catalogue summary:'];
  let total = 0;
  ...
  lines.push(`Total listed products: ${total}`);
  return lines.join('\n');
}
